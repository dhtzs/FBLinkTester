...
Help() {
  ...
  echo "        (output: \"${tool_dir}/deeplinks_custom/\")"
  echo "    -e, --extract"
  echo "        Extract Facebook APK file from connected Android device"
  echo "        (output: \"${tool_dir}/bin/APKs/\")"
  echo "    -s, --save"
  echo "        Save deeplinks for inspection"
  echo "        (output: \"${tool_dir}/deeplinks_saved/\")"
  echo "    -u, --update"
...
Check_Device() {
  ...
  if [[ -z "$(adb devices | grep 'device$')" ]]; then
	echo -e "\n╰─> ${font_red}${font_underline}Error${font_red}:${font_reset} No Android device connected."
	exit 1
  fi
  # Check if Facebook application is installed on Android device
  if [[ -z "$(adb shell pm list packages | grep 'com.facebook.katana')" ]]; then
	echo -e "\n╰─> ${font_red}${font_underline}Error${font_red}:${font_reset} Facebook application not installed on connected Android device."
	exit 1
  fi
}
